...
const VendorWithdrawalHistory = () => {
  ...
  const downloadCsv = () => {
    ...
    link.setAttribute(
      "download",
      `vendor-withdrawals-${new Date().toISOString().slice(0, 10)}.csv`
    );
  ...
  useEffect(() => {
    const fetchRows = async () => {
      ...
      if (!token) {
        setError("You need to log in as a vendor to view withdrawal history.");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/vendor/wallet/withdrawals`,
          {
    ...
    <div style={{ padding: 24 }}>
      ...
      >
        ...
        <Space>
          ...
          <Button onClick={() => navigate("/vendors/wallet")}>Back to Wallet</Button>
        </Space>
      </Space>
